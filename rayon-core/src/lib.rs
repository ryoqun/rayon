...
pub struct ThreadPoolBuilder<S, C: crossbeam_deque::CustomCollector> {
...
impl<C: CustomCollector> Default for ThreadPoolBuilder<DefaultSpawn, C> {
    fn default() -> Self {
        ThreadPoolBuilder::<DefaultSpawn, C> {
            num_threads: 0,
            panic_handler: None,
...
impl Configuration {
    ...
    pub fn build(self) -> Result<ThreadPool<DefaultCollector>, Box<dyn Error + 'static>> {
    ...
    fn into_builder(self) -> ThreadPoolBuilder<DefaultSpawn, DefaultCollector> {
        self.builder
    }
}
