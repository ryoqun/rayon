...
use crossbeam_deque::{Injector, Steal, Stealer, Worker, DefaultCollector, CustomCollector};
use std::any::Any;
use std::cell::Cell;
...
pub trait ThreadSpawn<C: CustomCollector = DefaultCollector> {
    private_decl! {}
...
impl<C: CustomCollector> WorkerThread<C> {
    ...
    pub(super) fn current() -> *const WorkerThread<C> {
        panic!();//WORKER_THREAD_STATE.with(Cell::get)
    }
...
unsafe fn main_loop<C: CustomCollector>(worker: Worker<JobRef, DefaultCollector>, registry: Arc<Registry<C>>, index: usize) {
    panic!();
    /*
